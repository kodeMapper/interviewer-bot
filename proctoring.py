...
people_detector = PeopleDetector(model_path="yolov8n.pt", device=DEVICE)

def log_violation(message):
    with open("proctoring_violation_log.txt", "a") as f:
...
camera_index = 1  # Default to internal camera
...
DARK_THRESHOLD = 45
LOOK_AWAY_TIMEOUT = 2.35  # 🔥 Alert if not looking for 3-4 seconds
...
def start_proctoring():
    ...
    while running:
        ...
        people_boxes = people_detector.detect(frame)
        num_people = len(people_boxes)
        if num_people > 1:
            current_status["status"] = "ALERT"
            current_status["reason"] = f"MULTIPLE PEOPLE DETECTED ({num_people})"
            log_violation("Multiple people detected in frame")
            start_buzzer()
            latest_frame = frame.copy()
            continue  # Skip rest of loop, prioritize this alert
